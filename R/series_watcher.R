...
for(r in seq(1:length(pairing_ranges))){
  round_pairs <- range_read(ss = season_key,
                         sheet = api_sheet,
...
for(p in seq(1:nrow(unplayed))){
  ...
  query <- httr::GET(
    ...
    query = list(since = as.character(formatC(as.numeric(lubridate::parse_date_time(season_start, "ymd")) * 1000, digits = 0, format = "f")),
                 until = "Now",
                 perfType = "classical",
                 vs = query_b,
                 clocks = "true"),
